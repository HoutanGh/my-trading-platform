...
- For breakout automation we add a method such as:
  - `place_buy_breakout(symbol, qty, entry_price, tp_price, sl_price)`  
    (exact signature to be finalized).
- That method:
...
- **Single‑fire**:
  - The breakout watcher is **single‑use** per configuration:
    ...
    - Then stops. No automatic re‑arming in v1.

- **Connection loss**:
  - Before entry is placed:
    - If the connection drops, we log and stop; we do not retroactively place a trade based on bars we might have missed.
  - After a bracket is placed:
    - IB holds TP/SL, so risk is still controlled.
    - On reconnect, we can query open orders/positions to rebuild state (future enhancement).
